...
const PRESETS: Preset[] = [
  {
    id: 'preset-1',
    name: 'Preset 1',
    shortName: 'Preset 1 ($5.00)',
    description: '100mm W × 90mm D × 80mm H',
    width: 100,
    depth: 90,
    height: 80,
    cost: 5.00,
  ...
  },
  {
    id: 'preset-2',
    name: 'Preset 2',
    shortName: 'Preset 2 ($8.00)',
    description: '100mm W × 100mm D × 100mm H',
    width: 100,
    depth: 100,
    height: 100,
    cost: 8.00,
  ...
  },
  {
    id: 'preset-3',
    name: 'Preset 3',
    shortName: 'Preset 3 ($3.00)',
    description: '80mm W × 80mm D × 50mm H',
    width: 80,
    depth: 80,
    height: 50,
    cost: 3.00,
...
export default function App() {
  // Current active CAD state (defaults to Preset 1: 100x90x80mm @ $5.00)
  ...
  // Helper to compute fabrication cost (fixed preset price or dynamic estimation)
  const getCostInfo = (w: number, d: number, h: number) => {
    const matching = PRESETS.find((p) => p.width === w && p.depth === d && p.height === h);
    if (matching) {
      return {
        cost: matching.cost,
        isPreset: true,
        presetName: matching.name,
        presetId: matching.id
      };
    }
    // Dynamic calculation for custom dimensions based on surface area
    const totalAreaMm2 = 2 * (w * d + w * h + d * h);
    const calculated = Math.max(1.50, Math.round((totalAreaMm2 * 0.0001033) * 100) / 100);
    return {
      cost: calculated,
      isPreset: false,
  ...
  const currentCostInfo = getCostInfo(params.width, params.depth, params.height);
  ...
  const loadPreset = (preset: Preset) => {
    ...
    setExplode(0);
    triggerFeedback(`Loaded ${preset.name} (${preset.width}×${preset.depth}×${preset.height}mm) — $${preset.cost.toFixed(2)}`, 'success');
  };
  ...
  const handleSendToStaff = async (senderName: string, senderEmail: string, notes: string, customFileName?: string) => {
    try {
      ...
      const costInfo = getCostInfo(params.width, params.depth, params.height);
      const defaultNotes = notes && notes.trim() !== ''
        ? notes
        : `Box Specs: ${params.width}x${params.height}x${params.depth}mm | Cost: $${costInfo.cost.toFixed(2)}${costInfo.isPreset ? ` (${costInfo.presetName})` : ''} | Material: ${selectedWood.toUpperCase()} (${params.thickness}mm) | Kerf: ${params.laserKerf}mm | Finger Width: ${params.fingerWidth}mm`;
  ...
  return (
    <div id="app-root" className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans overflow-hidden select-none">
      ...
      <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between px-6 py-3 bg-white border-b border-slate-200 shrink-0 gap-4">
        ...
        {/* CAD Preset Quick Picker with Costs */}
        <div className="flex items-center gap-1.5 flex-wrap bg-slate-50 p-1.5 rounded-xl border border-slate-200">
          <span className="text-[10px] font-extrabold text-slate-500 px-2 uppercase tracking-widest flex items-center gap-1">
            <Tag className="w-3.5 h-3.5 text-blue-600" /> Presets:
          </span>
          {PRESETS.map((preset) => {
            const isSelected = params.width === preset.width && params.depth === preset.depth && params.height === preset.height;
            return (
              <button
                key={preset.id}
                onClick={() => loadPreset(preset)}
                className={`text-xs font-bold rounded-lg px-2.5 py-1.5 transition-all cursor-pointer border flex items-center gap-1.5 shadow-xs ${
                  isSelected
                ...
                title={`${preset.name}: ${preset.width}W × ${preset.depth}D × ${preset.height}H mm - $${preset.cost.toFixed(2)}`}
              >
                <span>{preset.name}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono font-extrabold ${
                  isSelected
        ...
        {/* Cost Badge & Global actions */}
        <div className="flex items-center gap-2.5">
          {/* Prominent Current Box Cost Badge */}
          <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg shadow-xs">
            ...
            <div className="flex flex-col">
              <span className="text-[9px] font-extrabold text-emerald-700 uppercase tracking-wider leading-none">
                {currentCostInfo.isPreset ? currentCostInfo.presetName : 'Estimated Cost'}
              </span>
              <span className="text-sm font-extrabold font-mono text-emerald-900 leading-tight">
                ${currentCostInfo.cost.toFixed(2)}
              </span>
            </div>
          </div>
          ...
          <button 
            onClick={() => {
              ...
              triggerFeedback("Reset to Preset 1 ($5.00)", "info");
            }}
        ...
        <aside className="w-full lg:w-[320px] shrink-0 bg-slate-50 border-r border-slate-200 flex flex-col p-5 gap-6 overflow-y-auto max-h-[calc(100vh-80px)] select-none">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="flex flex-col gap-2">
              {PRESETS.map((preset) => {
                const isActive = params.width === preset.width && params.depth === preset.depth && params.height === preset.height;
                return (
                  <div
                    key={preset.id}
                    onClick={() => loadPreset(preset)}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-extrabold font-mono text-emerald-700 bg-emerald-100/90 px-2 py-0.5 rounded border border-emerald-200">
                        ${preset.cost.toFixed(2)}
                      </span>
                    </div>

                    <div className="flex justify-between items-center text-[10px] text-slate-500 font-mono pl-5.5">
                      <span>{preset.width}W × {preset.depth}D × {preset.height}H mm</span>
                      {isActive ? (
                        <span className="text-blue-600 font-bold font-sans flex items-center gap-1">
                          <Check className="w-3 h-3 stroke-[2.5]" /> Active
                        </span>
                      ) : (
                        <span className="text-slate-400 font-sans hover:text-blue-600">Select →</span>
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="grid grid-cols-1 gap-2">
              {[
                ...
                },
                {
                  ...
                  desc: 'choose from 2 custom removable lid profiles'
                }
              ].map((style) => (
                <button
                  key={style.id}
        ...
        <section className="flex-grow flex flex-col min-w-0 bg-white p-6 gap-5 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-250 pb-3 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 py-1.5 px-3.5 rounded-lg text-xs font-mono text-slate-600">
              ...
              <span className="text-slate-300">|</span>
              <span className="text-emerald-700 font-bold font-mono bg-emerald-100/90 px-1.5 py-0.5 rounded border border-emerald-200">
                ${currentCostInfo.cost.toFixed(2)}
              </span>
            </div>
          </div>
